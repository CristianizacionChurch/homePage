...
function toPrayerRow(prayer) {
    const fecha = new Date(prayer.fecha).toLocaleDateString('es-DO');
    return [fecha, prayer.nombre, prayer.peticion];
}

async function appendPrayerRecord(spreadsheetId, prayer, deps = {}) {
    const sheets = deps.sheets || getSheets();
    if (!sheets) throw new Error('Google Sheets not configured');
    await sheets.spreadsheets.values.append({
        spreadsheetId,
        range: 'A1',
        valueInputOption: 'USER_ENTERED',
        insertDataOption: 'INSERT_ROWS',
        requestBody: { values: [toPrayerRow(prayer)] }
    });
}
...
module.exports = { appendRecord, getRecords, toSheetRow, fromSheetRow, toPrayerRow, appendPrayerRecord };
