...
async function getFileStream(fileId, drive = getDrive()) {
    if (!drive) throw new Error('Drive not configured');
    const res = await drive.files.get({ fileId, alt: 'media' }, { responseType: 'stream' });
    return res.data;
}

async function getFileName(fileId, drive = getDrive()) {
    if (!drive) throw new Error('Drive not configured');
    const res = await drive.files.get({ fileId, fields: 'name' });
    return res.data.name;
}

module.exports = { getDrive, resizeThumb, listAll, listSections, getFileStream, getFileName };
